...
- **先写测试，再写实现**
- **测试即文档**：测试用例 = 组件行为规范
- **每个任务 5 步**：明确目标 → 写测试 → 审核测试 → 写实现 → 跑测试通过

### 4.1.1 工作流规范（强制）

| 规则 | 说明 |
|------|------|
| **分支规范** | 禁止直接在 master 提交，所有开发在 feature 分支进行，审核后合并 |
| **SPEC 变更审核** | spec.md 的任何修改须经开发者审核通过后方可编码 |
| **测试审核** | 测试文件编写完成后须经开发者审核，确认测试逻辑正确 |
| **测试头部规范** | 每个测试文件顶端须包含审核人、日期、审核状态 |
